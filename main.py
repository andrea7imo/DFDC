...
training_set = Dataset('/aiml/project/DFDC/FramesDataset_full/train', transform=train_transform, max_real=None, max_fake=18663, over=False)
...
model, *_ = model_selection('xception', num_out_classes=2)
...
best_epoch, best_model_wts, bestAccuracy, bestF_1, accuracies, accuraciesTrain, F_1s, loss_values = \
    train(model, train_dataloader, valid_dataloader, type_optimizer)

saveModel(best_epoch, best_model_wts, loss_values, accuracies, accuraciesTrain, F_1s,
          f'/aiml/project/DFDC/Outputs/models/model_over-all-{NUM_EPOCHS}-fine_adam_best-xcept.pth')
                        # model name format: model_<max_real>-<max_fake>-<NUM_EPOCHS>-<hyp_id>[-<trans>][-<xcept>].pth
...
net, best_epoch, loss_values, accuracies, accuraciesTrain, f1s, best_model_wts = \
    loadModel('/aiml/project/DFDC/Outputs/models/model_all-18663-20-fine_adam_best-trans.pth')
